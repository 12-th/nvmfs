...
struct inode * demofs_alloc_inode(struct super_block * pSuperBlock,
	const struct inode * pDirNode, umode_t mode, dev_t dev)
{
	//allocate a new inode in memory. if we have set the alloc_inode callback in super_operations, it will be called.
	//otherwise, it will just call kmem_cache_alloc and do some initialize work(set i_nlink, i_sb, i_blkbits, i_dev...).
	...
	if (pInode)
	{
		...
		pInode->i_atime = pInode->i_mtime = pInode->i_ctime = CURRENT_TIME;
...
static int demofs_mknod(struct inode * pDirNode, struct dentry * pDentry, umode_t mode, dev_t dev)
{
	...
	if (pInode)
	{
		...
		pDirNode->i_mtime = pDirNode->i_ctime = CURRENT_TIME;
		return 0;
	}
...
static int demofs_symlink(struct inode * pNodeDir, struct dentry * pDentry, const char * symname) 
{
	...
	if (pInode)
	{
		...
		if (!err)
		{
			...
			pNodeDir->i_mtime = pNodeDir->i_ctime = CURRENT_TIME;
		}
		else
			iput(pInode);
...
static const struct super_operations demofsSuperOps = {
...
int demofs_fill_sb(struct super_block * pSuperBlock, void * options, int silent)
{
	...
	save_mount_options(pSuperBlock, options);

	pFSInfo = kzalloc(sizeof(struct STDemoFSInfo), GFP_KERNEL);
	if (!pFSInfo)
		return -ENOMEM;

	if (err = demofs_parse_options(options, &pFSInfo->m_mountOpt))
		return err;

	pSuperBlock->s_fs_info = pFSInfo;

	pSuperBlock->s_maxbytes = MAX_LFS_FILESIZE;
	pSuperBlock->s_blocksize = PAGE_CACHE_SIZE;
	pSuperBlock->s_blocksize_bits = PAGE_CACHE_SHIFT;
