...
TEST(LayouterTest, InitTest)
{
    ...
    const UINT64 SWAP_TABLE = PAGE_UNMAP_TABLE + (344UL << 20);
    const UINT64 BLOCK_SWAP_TRANSACTION_LOG_AREA =
        SWAP_TABLE + AlignUpBits((SWAP_TABLE_BLOCK_NUM << BITS_2M) + (SWAP_TABLE_PAGE_NUM << BITS_4K), BITS_2M);
    const UINT64 PAGE_SWAP_TRANSACTION_LOG_AREA =
        BLOCK_SWAP_TRANSACTION_LOG_AREA + BLOCK_SWAP_TRANSACTION_LOG_AREA_SIZE;
    const UINT64 MAP_TABLE_SERIALIZE_DATA = PAGE_SWAP_TRANSACTION_LOG_AREA + PAGE_SWAP_TRANSACTION_LOG_AREA_SIZE;
    ...
    EXPECT_EQ(l.blockSwapTransactionLogArea, BLOCK_SWAP_TRANSACTION_LOG_AREA);
    EXPECT_EQ(l.pageSwapTransactionLogArea, PAGE_SWAP_TRANSACTION_LOG_AREA);
